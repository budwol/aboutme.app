...
The check suite spans several slow, different-shaped stages: formatting, unit tests, integration tests, a dry-run of `init.sh`, a smoke export, and Playwright e2e — each with different failure modes and different value in catching a broken PR early versus late.

## Decision

Each concern is its own reusable workflow file under `.github/workflows/` (`ci-lint-prettier.yml`, `ci-unit.yml`, `ci-integration.yml`, `ci-dry-run.yml`, `ci-smoke.yml`, `ci-e2e.yml`), declared with `on: workflow_call`. `ci.yml` is the entry point triggered on push/PR and chains them via job-level `needs:` in cheapest-and-most-likely-to-fail-first order: `lint_prettier → unit → integration → dry_run → smoke → e2e`. `package.json`'s `npm run test:all` and `npm run ci:local` mirror roughly the same order locally.
...
- No workflow currently runs `jest --coverage`. [ADR 0013](0013-100-percent-line-and-branch-coverage-target.md) sets a 100% line/branch coverage target and enforces it locally via `coverageThreshold`, but deliberately doesn't wire it into `ci_unit` (or any other workflow) yet, since the current baseline is well below 100% — adding a `ci_coverage` workflow_call step, chained after `unit`, is the natural next step once that gap is closed.
