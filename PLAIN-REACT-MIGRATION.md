...
Fortschritt: `[####################]` 100% (4 von 4 Phasen abgeschlossen)

Status: **abgeschlossen** (Phase 1, 2, 3 und 4 abgeschlossen). Dieses
...
Status: **abgeschlossen** (2026-09-15) · Risiko: mittel · setzt Phase 2 voraus (Routing darf
...
| 2026-09-15 | 3 | Erster Teilschritt: `jest.config.cjs`s `preset: "jest-expo"` durch `ts-jest` + globalem `testEnvironment: "jsdom"` ersetzt (vorher: `jest-environment-node` unter der Haube, kein echtes DOM); `tsconfig.json`s `extends: "expo/tsconfig.base"` durch eine inline plain-web-Konfiguration ersetzt (`"jsx": "react-jsx"` statt `"react-native"`, `customConditions: ["react-native"]` entfernt); `app.config.ts`/`expo-constants`-Abhängigkeit aus `currentAppVersion.ts` entfernt (liest jetzt eine build-time-injizierte `__APP_VERSION__`-Konstante, die Vite später per `define` setzt, Jest setzt sie in `jest.setup.cjs` aus `package.json`). Die jsdom-Umstellung deckte mehrere zuvor durch das dokumentfreie Test-Environment maskierte SSR-Weichen auf (`typeof document/window === "undefined"`-Zweige in `WnaApp.tsx`, `WnaDrawerLayout.tsx`, `app-data/index.ts`, sowie `WnaPrivateRepoModal`s `createPortal`-Zweig, der mit `react-test-renderer` grundsätzlich nicht in ein echtes DOM portalen kann) — jeweils durch gezielte Tests/Mocks statt Produktionscode-Änderung geschlossen, da das Verhalten in echten Browsern bereits korrekt war | `test:prettier`, `lint`, `test:types`, `test:circular`, `test:unit` (603/603), `test:coverage` (100 %), `test:integration` (31/31) — alle mehrfach wiederholt, grün. `test:e2e`/`test:smoke`/`web`/`export:web` noch offen (hängen an der noch ausstehenden Vite-Umstellung) |
| 2026-09-15 | 3 | Phase 3 vollständig umgesetzt: `vite.config.ts` (Pfad-Aliase, `envPrefix: "EXPO_PUBLIC_"`, `__APP_VERSION__`-`define` aus `package.json`, ein kleines Plugin ersetzt `%WNA_BACKGROUND_PRELOAD_URL%` in `index.html` durch die versionierte Hintergrundbild-URL) ersetzt `metro.config.js` (gelöscht); `index.html` im Projekt-Root ersetzt `src/app/+html.tsx` (Inhalt unverändert übernommen, inkl. `#root{display:flex}` als expliziter Ersatz für `expo-router/html`s `<ScrollViewStyleReset/>`, die `#wna-safe-area`s `flex:1` sonst wirkungslos gemacht hätte); `src/main.tsx` (neuer Vite-Einstiegspunkt, mountet `WnaRootLayout` in `#root`) und eine neue echte `WnaErrorBoundary`-Klassenkomponente (fängt Render-Fehler ab und rendert `WnaApp.tsx`s bestehende `ErrorBoundary`-Fallback-UI) ersetzen den Fehler-Catching-Mechanismus, den bisher `expo-router` implizit bereitstellte; `app.config.ts` gelöscht; komplettes `src/app/` (18 Routen-Stubs + `_layout.tsx`/`index.tsx`/`+not-found.tsx`/`+html.tsx` + deren Tests) gelöscht, da `WnaRoutes` bereits jeden Pfad selbst matcht und `nginx/site.conf`s vorhandenes `try_files $uri /index.html` einen Single-Entry-Export ohnehin schon unterstützt; `EXPO_PUBLIC_DEPLOY_VERSION`-Lesestelle in `versionedAssetUrl` auf `import.meta.env` umgestellt (über eine neue `@utils/publicEnv`-Indirektion, da Jests CommonJS-Runtime `import.meta` nicht parsen kann — per `moduleNameMapper` auf ein `publicEnv.jest.ts`-Gegenstück mit `process.env` umgeleitet); `getConfiguredSiteUrl()` (`appConfig`) ersatzlos entfernt, da sie außerhalb ihres eigenen Tests nirgends aufgerufen wurde und ihre `BASE_URL`-Hälfte unter Vite ohnehin nie funktioniert hätte (nur `EXPO_PUBLIC_*`-Variablen werden ins Bundle injiziert); `npm run web`/`export:web`, `scripts/start-e2e-web.sh`, `scripts/ci-local.sh` (kein `expo-doctor` mehr), `scripts/smoke-export.sh` (kein `expo config`/`expo export` mehr) auf Vite umgestellt; `scripts/assert-web-bundle.cjs` auf `dist/assets/` (statt `dist/_expo/static/js/web/`) umgestellt, `FORBIDDEN_MARKERS` um `expo-router`/`react-native` erweitert; `scripts/assert-coverage.cjs` erkennt jetzt reine Typ-Dateien (nur `interface`/`type`/`import`-Statements) und verlangt für sie keinen Coverage-Eintrag mehr, da `ts-jest` (anders als vorher `babel-jest`) für sie keinen erzeugt | Vollständiger `npm run export:web`-Lauf (der intern `ci-local.sh` inkl. `test:e2e` gegen den echten Vite-Dev-Server ausführt) einmal komplett grün durchlaufen: `test:prettier`, `lint`, `test:types`, `test:circular`, `test:unit` (584/584), `test:coverage` (100 %, inkl. `assert-coverage.cjs`), `test:integration` (31/31), `test:dry-run`, `test:smoke` (Bundle-/PWA-Check gegen den echten `vite build`-Output), `test:e2e` (47/47) — danach `test:e2e` noch zweimal isoliert wiederholt, `npm run web` manuell gegen einen echten Port verifiziert (HTTP 200). Zwei Formatierungs-Annahmen mussten nachträglich korrigiert werden: Prettier bricht lange HTML-Attribute auf mehrere Zeilen um, wodurch `assert-web-pwa.cjs`s und `smoke-export.sh`s Ein-Zeilen-`grep` auf das `robots`-Meta-Tag nicht mehr trafen (beide auf toleranteres Matching umgestellt) |

| 2026-09-15 | 4 | Phase 4 vollständig umgesetzt — Migration abgeschlossen: `expo`, `expo-router`, `expo-constants`, `expo-linking`, `expo-doctor`, `eslint-config-expo`, `jest-expo`, `react-native`, `react-native-web`, `@react-navigation/native`, `react-native-safe-area-context`, `react-native-screens`, `react-native-gesture-handler`, `react-native-reanimated`, `react-native-worklets` aus `package.json` entfernt (508 Pakete aus `node_modules`, `npm ls --depth=0` danach ohne jedes Native-/Expo-Paket). `eslint.config.cjs`s `compat.extends("expo", "prettier")` durch reine Flat-Config-Registrierung von `@eslint/js`, `eslint-plugin-react`, `eslint-plugin-react-hooks`, `eslint-plugin-import` + `@typescript-eslint` ersetzt (dabei u. a. `no-require-imports` korrekt auf `.ts`/`.tsx` beschränkt, `import/no-named-as-default-member` wegen bekannter i18next-Fehlalarme deaktiviert, tote `./src/app`-Zonen aus der ADR-15-Regel entfernt). `.gitignore` um `.expo/`, `.expo-shared/`, `web-build/`, `.metro-health-check*`, `expo-env.d.ts` bereinigt. Die Entfernung deckte einen echten, seit Phase 3 unbemerkten Laufzeitfehler auf: `src/utils/logger.ts` nutzte weiterhin das React-Native-Bundler-Global `__DEV__`, das unter Vite nie existiert hat (nur zufällig noch über `@types/react-native`s Ambient-Deklaration kompiliert) — durch `isDevMode()` (`@utils/publicEnv`, Vite: `import.meta.env.DEV`, Jest: `process.env.NODE_ENV`) ersetzt. Mehrere vestigiale `jest.mock("expo-router"/"react-native", ...)`-Blöcke ohne jeden Produktionscode-Bezug in Unit-/Integrationstests entfernt (u. a. `WnaHomeRoute.test.tsx`, drei Integrationstests, ein `FlatList`-Mock in `projects.integration.test.tsx`). Die Lizenzseite (`legalContent.ts`) listete noch `React Native`, `React Native Web`, `Expo`, `Expo Router`, `React Navigation`, `React Native Reanimated/Gesture Handler/Screens/SVG`, `Async Storage` als verwendete Open-Source-Komponenten — durch die tatsächlich genutzten (`React`, `React DOM`, `Vite`, `i18next`/`react-i18next`, `sanitize-html`, `Material Design Icons`) ersetzt; dabei auch eine vorbestehende Falschangabe korrigiert (`@mdi/js` ist Apache-2.0, nicht MIT, wie zuvor gelistet). ADR 21 (Source-Maps) und die zugehörigen `EXPO_OWNER`/`EXPO_SLUG`/`EXPO_PUBLIC_ENABLE_SOURCE_MAPS`-Reste in `.env.example`/`process-env.d.ts` auf den heutigen Vite-Mechanismus (`build.sourcemap` bleibt unset/`false`) aktualisiert | Vollständiger `npm run export:web`-Lauf (inkl. `ci-local.sh`s `test:e2e` gegen den echten Vite-Dev-Server) grün: `test:prettier`, `lint`, `test:types`, `test:circular`, `test:unit` (586/586), `test:coverage` (100 %), `test:integration` (31/31), `test:dry-run`, `test:smoke`, `test:e2e` (47/47) sowie separat `test:security`, `test:deps`. Lighthouse lokal gegen den echten `vite build`-Output gemessen (Chrome headless via `npx lighthouse`): Performance 97, Accessibility 100, Best Practices 100, SEO 66 — exakt identisch zur Baseline vom 2026-09-14 gegen die Produktionsseite; keine Regression durch die gesamte Migration |
