...
describe("WnaCardVerticalSmall", () => {
  ...
  it("skips re-rendering while memoized props stay stable", () => {
    ...
    act(() => {
      tree = TestRenderer.create(<WnaCardVerticalSmall {...props} />);
    });

    expect(mockWnaCardTextContent).toHaveBeenCalledTimes(1);

    act(() => {
      tree!.update(<WnaCardVerticalSmall {...props} />);
    });
    ...
    act(() => {
      tree!.update(
        <WnaCardVerticalSmall {...props} description="Changed features." />,
      );
    });
